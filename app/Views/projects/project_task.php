...
<div style="min-height: 100vh; padding: 2rem; font-family: 'Roboto', sans-serif;">
    <div style="max-width: 900px; margin: 0 auto;">
        <div class="d-flex justify-content-between align-items-center mb-4">
            <h2 style="font-family: 'Poppins', sans-serif; font-weight: 700; color: #1f2937;">Project Scope</h2>
            <div class="d-flex gap-2">
                <button class="btn btn-primary" id="addScopeBtn">
                    <i class="fas fa-plus"></i> Add Project Scope
                </button>
            </div>
        </div>
...
$(document).ready(function() {
        ...
        $.ajax({
            ...
            success: function(res) {
                ...
                if (res.success && Array.isArray(res.scopes) && res.scopes.length > 0) {
                    ...
                    $('#scopeList').html('<div class="text-center text-muted" style="padding:2rem;">No scopes for this project.<br><button class="btn btn-primary mt-3" id="createScopeBtn"><i class="fas fa-plus"></i> Create Scope</button></div>');
                }
            },
    ...
    function renderScope(scope) {
        const scopeCard = `
            <div class="scope-card mb-4" data-scope-id="${scope.id}" style="background: #fff; border-radius: 1rem; border: 1px solid #e2e8f0; padding: 1.5rem;">
                <div class="scope-header d-flex justify-content-between align-items-center mb-3">
                    <div class="scope-info">
                        <h4 style="font-family: 'Poppins', sans-serif; font-weight: 700; color: #1f2937; margin: 0;">${scope.name}</h4>
                    </div>
                    <div class="scope-actions d-flex gap-2">
                        <button class="btn btn-sm btn-outline-primary add-components-btn" data-scope-id="${scope.id}">
                            <i class="fas fa-plus"></i> Add Components
                        </button>
                        <button class="btn btn-sm btn-outline-secondary edit-scope-btn" data-scope-id="${scope.id}">
                            <i class="fas fa-cog"></i> Edit
                        </button>
                    </div>
                </div>
                <div class="components-container" id="components-${scope.id}">
                                    ...
                                    <div class="progress-indicator d-flex flex-column align-items-center">
                                        <div style="width:24px; height:24px; position:relative;">
                                            <svg width="24" height="24" viewBox="0 0 24 24">
                                                <circle cx="12" cy="12" r="9" stroke="#e2e8f0" stroke-width="3" fill="none" />
                                                <circle cx="12" cy="12" r="9" stroke="#06b6d4" stroke-width="3" fill="none" stroke-dasharray="${Math.PI * 2 * 9}" stroke-dashoffset="${Math.PI * 2 * 9 * (1 - percent / 100)}" stroke-linecap="round" transform="rotate(-90 12 12)" />
                                            </svg>
    ...
    function updateComponentOrder(scopeId) {
        ...
        $.ajax({
            url: '<?= base_url('projects/update_template_order') ?>',
            method: 'POST',
            data: {
                scope_id: scopeId,
    ...
    $(document).on('click', '#addScopeBtn, #createScopeBtn', function() {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: '<?= base_url('projects/create_scope') ?>',
                    method: 'POST',
                    data: {
                        project_id: projectId,
        ...
        Swal.fire({
            ...
            didOpen: () => {
                ...
                $('#editComponentsList').on('click', '.save-component-name', function() {
                    ...
                    $.ajax({
                        url: '<?= base_url('projects/update_component_name') ?>',
                        method: 'POST',
                        data: {
                ...
                $('#editComponentsList').on('click', '.remove-component-from-edit', function() {
                    ...
                    Swal.fire({
                        ...
                        if (result.isConfirmed) {
                            $.ajax({
                                url: '<?= base_url('projects/soft_delete_component') ?>',
                                method: 'POST',
                                data: {
                                    template_id: templateId,
                                    project_id: projectId
                ...
                $('#deleteScopeFromEdit').on('click', function() {
                    Swal.fire({
                        ...
                        if (deleteResult.isConfirmed) {
                            $.ajax({
                                url: '<?= base_url('projects/delete_scope') ?>',
                                method: 'POST',
                                data: { 
                                    scope_id: scopeId,
                                    project_id: projectId
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: '<?= base_url('projects/update_scope') ?>',
                    method: 'POST',
                    data: {
                        scope_id: scopeId,
                        name: result.value.name,
    ...
    $('#scopeList').on('click', '.add-components-btn', function() {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed && result.value.length > 0) {
                ...
                $.ajax({
                    url: '<?= base_url('projects/add_custom_template_to_scope') ?>',
                    method: 'POST',
                    data: {
                        scope_id: scopeId,
                        project_id: projectId,
